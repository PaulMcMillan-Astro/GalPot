...
public:
    ...
    Matrix() {}
    Matrix(const T);
    Matrix(const Matrix& M) { for(register int i=0;i<N1*N2;i++) a[i]=M.a[i];}
   ~Matrix() {}
...
inline Matrix<T,N1,N2>::Matrix(const T fill_value)
    { for(register int i=0; i<N1*N2; i++) a[i] = fill_value; }
...
inline Matrix<T,N1,N2>& Matrix<T,N1,N2>::operator= (const Matrix<T,N1,N2>& M)
    { for(register int i=0; i<N1*N2; i++) a[i] = M.a[i];
      return *this; }
...
inline Matrix<T,N1,N2>& Matrix<T,N1,N2>::operator+= (const T m)
    { for(register int i=0; i<N1*N2; i++) a[i] += m;
      return *this; }
...
inline int Matrix<T,N1,N2>::operator== (const Matrix<T,N1,N2>& M) const
    { for(register int i=0; i<N1*N2; i++) if(a[i] != M.a[i]) return 0;
      return 1; }

template<class T, int N1, int N2>
inline int Matrix<T,N1,N2>::operator!= (const Matrix<T,N1,N2>& M) const
    { for(register int i=0; i<N1*N2; i++) if(a[i] != M.a[i]) return 1;
      return 0; }
...
inline Vector<T,N1> Matrix<T,N1,N2>::column(const int n2) const
    { Vector<T,N1> P; register int i,j;
...
inline Vector<T,N2> Matrix<T,N1,N2>::row(const int n1) const
    { Vector<T,N2> P; register int i, j;
...
inline void Matrix<T,N1,N2>::fill_row(const T fill_value, const int n1)
    { if(n1>=N1 || n1<0) range_error();
      for(register T*b=a+n1*N2; b<a+(n1+1)*N2; b++) *b = fill_value; }

template<class T, int N1, int N2>
inline void Matrix<T,N1,N2>::multiply_row(const T multiplier, const int n1)
    { if(n1>=N1 || n1<0) range_error();
      for(register T*b=a+n1*N2; b<a+(n1+1)*N2; b++) *b *= multiplier; }
...
inline void set_to_unity(Matrix<T,N,N>& M)
{
    ...
    M = zero;
    for(register int i=0; i<N; i++) M[i][i] = one;
}
...
{ 
    register T t = T(0);
    for(register int i=0; i<N; i++) t += M(i,i); 
    return t;
}
...
{
    register T y,*pa=A[0],*pai,*pb,*pbi, *pc=C[0];
    register int l,m,n;
    for(l=0; l<L; l++,pa+=M)
...
// on return C = A*x
{
    register T *pa=A[0],*pc=C[0],*aup=pa+N1*N2;
    for(; pa<aup; pa++,pc++)
	if(*pa) *pc = *pa * x;
}
